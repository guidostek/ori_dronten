...
def create_google_doc():
    ...
    if not fractie_date:
        print("Geen relevante vergadering gevonden.")
        return
    ...
    # Opbouw van de volledige tekst (startend met een enter voor het logo erboven)
    full_text = "\n" 
    ...
    # Stap 1: Alleen de tekst invoegen
    requests = [{'insertText': {'location': {'index': 1}, 'text': full_text}}]
    
    # Stap 2: Linkjes plaatsen op basis van de originele tekstindexen (voordat er een logo is!)
    ...
    # Stap 3: De titel opmaken (Titel begint na de eerste "\n", dus op index 2)
    requests.append({
        'updateTextStyle': {
            'range': {'startIndex': 2, 'endIndex': 2 + len(title_line)},
            'textStyle': {
                'foregroundColor': {'color': {'rgbColor': {'red': 1.0, 'green': 0.39, 'blue': 0.0}}},
    ...
    # Stap 4: PAS ALS LAATSTE het logo invoegen en rechts uitlijnen
    requests.extend([
        {'insertInlineImage': {
            'location': {'index': 1},
            'uri': LOGO_URL,
            'objectSize': {'height': {'magnitude': 75, 'unit': 'PT'}, 'width': {'magnitude': 75, 'unit': 'PT'}}
        }},
    ...
    print(f"Gereed! Logo staat rechtsboven, titel is oranje, en de linkjes werken perfect.")
