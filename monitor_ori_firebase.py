...
def run_monitor():
    ...
    try:
        ...
        for meeting in meetings:
            ...
            # Sla de vergadering op INCLUSIEF de agendapunten en de nieuwe API data
            db.collection('vergaderingen').document(m_id).set({
                'id': int(m_id),
                'title': title,
                'date': m_date,
                'startTime': meeting.get('startTime', ''), # Nieuw: Tijd!
                'confidential': is_geheim,
                'description': meeting.get('description', ''), # Nieuw: Voor de subtitel
                'dmu': meeting.get('dmu'),                     # Nieuw: Bevat de naam (bijv. Presidium)
                'meetingLabel': meeting.get('meetingLabel'),   # Nieuw: Bevat het label (bijv. Begroting)
                'location': meeting.get('location'),           # Nieuw: Locatie informatie
